...
Generation command (run from the repository root, replacing `OUTPUT` with a
fresh temporary path):

```sh
/usr/bin/ffmpeg -hide_banner -loglevel error \
  -f lavfi -i 'testsrc2=size=320x180:rate=24:duration=2' \
  -an -c:v libvpx -threads 1 -deadline good -cpu-used 0 \
  -crf 30 -b:v 0 -flags +bitexact -fflags +bitexact OUTPUT
```
...
```sh
/usr/bin/ffprobe -v error -show_entries stream=codec_name,width,height,r_frame_rate,duration \
  -of default=noprint_wrappers=1 mb/test/pages/pattern.webm
```
